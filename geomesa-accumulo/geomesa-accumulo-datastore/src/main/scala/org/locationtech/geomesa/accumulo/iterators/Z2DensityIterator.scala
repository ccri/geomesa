...
class Z2DensityIterator extends KryoLazyDensityIterator {
  ...
  override def writeNonPoint(geom: Geometry, weight: Double, result: DensityResult): Unit = geom match {
    ...
    case _ =>
      ...
      var i = 0
      while (i < Z2Index.GEOM_Z_NUM_BYTES) {
...
object Z2DensityIterator {
  ...
  def configure(sft: SimpleFeatureType,
                ...
                hints: Hints,
                priority: Int = KryoLazyDensityIterator.DEFAULT_PRIORITY, config: IndexConfig = DefaultConfig): IteratorSetting = {
    val is = KryoLazyDensityIterator.configure(sft, Z2Index(config), filter, hints, priority)
