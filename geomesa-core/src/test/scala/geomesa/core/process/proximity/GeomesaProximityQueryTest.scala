...
class GeomesaProximityQueryTest extends Specification {
   ...
   // are requesting a mock data store connection to Accumulo
    ...
    def getPoint(lat: Double, lon: Double, meters: Double) = {
      ...
      geoFac.createPoint(new Coordinate(dest2D.getX, dest2D.getY))
    }
   ...
   "GeomesaProximityQuery" should {
     "find things close by" in {
       ...
       WKTUtils.read("POINT(45 45)").bufferMeters(99).intersects(p1) should be equalTo true
       ...
       val prox = new GeomesaProximitySearch
       prox.execute(inputFeatures, dataFeatures, 99).size should be equalTo 0
  ...
  "GeomesaProximityQuery" should {
    "work on non-accumulo feature sources" in {
      ...
      val prox = new GeomesaProximitySearch
      prox.execute(inputFeatures, nonAccumulo, 99).size should be equalTo 0
