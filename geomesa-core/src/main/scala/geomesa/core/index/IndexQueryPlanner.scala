...
//    val ecql = Option(ECQL.toCQL(rewrittenCQL))
    ...
    if (iteratorConfig.useSFFI) {
      configureSimpleFeatureFilteringIterator(bs, featureType, ecql, query, goodPoly)
    }
  ...
  def tweakFilter(filter: Filter) = {
    filter match {
      ...
      // JNH: Fix vBSPO first. case op: BinarySpatialOperator => visitBinarySpatialOp(op)
      case _ => filter
    }
  }
  ...
  def doCorrectSpatialCall(op: BinarySpatialOperator, property: String, geom: Geometry): Filter = op match {
    ...
    // General, construct new filter based on old one?
  }
