...
package object filter {
  ...
  def partitionGeom(filter: Filter): (Seq[Filter], Seq[Filter]) = partitionSubFilters(filter, spatialFilters)

  def partitionTemporal(filter: Filter): (Seq[Filter], Seq[Filter]) = partitionSubFilters(filter, temporalFilters)
  ...
  // NB: This may need to be 'smaller' as we may wish to handle the various temporal predicates more carefully.
