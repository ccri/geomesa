...
import org.locationtech.geomesa.kafka09.common.ZkUtils
...
class ConsumerRebalancer[K, V](consumer: KafkaConsumer[K, V], config: ConsumerConfig)
  ...
  private val zkUtils = new KafkaUtils08().createZkUtils(config)
  ...
  private def rebalance(cluster: Seq[Broker]): Boolean = {
    ...
    if (brokers.size == 0) {
      ...
      val partitionOwnershipDecision = new KafkaUtils08().assign(partitionAssignor, assignmentContext)
