...
trait KafkaDataStoreSchemaManager extends DataStore with LazyLogging {
  ...
  def waitUntilTopicReady(zkUtils: ZkUtils09, topic: String, partitions: Int, timeToWait: Long = 5000L): Unit = {
    (0 until partitions).foreach { waitUntilTopicReadyForPartition(zkUtils, topic, _, timeToWait) }
  }

  def waitUntilTopicReadyForPartition(zkUtils: ZkUtils09, topic: String, partition: Int, timeToWait: Long) {
  ...
  private val zkUtils = {
    val ret = new KafkaUtils09().createZkUtils(zookeepers, Int.MaxValue, Int.MaxValue)
