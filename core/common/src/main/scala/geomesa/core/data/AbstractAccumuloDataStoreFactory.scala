...
abstract class AbstractAccumuloDataStoreFactory(val ops: VersionSpecificOperations) extends DataStoreFactorySpi {
  ...
  import AccumuloDataStoreFactoryHelper._
  import params._
...
object AccumuloDataStoreFactoryHelper {
  ...
  object params {
    ...
    val idxSchemaParam = new Param("indexSchemaFormat",
                                    ...
                                    false)
    val mockParam = new Param("useMock",
                               classOf[String],
  ...
  def getMRAccumuloConnectionParams(conf: Configuration): JMap[String, AnyRef] =
    Map[String, AnyRef](zookeepersParam.key -> conf.get(ZOOKEEPERS),
