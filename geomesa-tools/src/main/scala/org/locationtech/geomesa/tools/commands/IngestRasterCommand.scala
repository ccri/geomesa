...
class IngestRasterCommand(parent: JCommander) extends Command with AccumuloProperties {
  ...
  override def execute() {
    val fmt = Option(params.format).getOrElse(getFileExtension(new File(params.file)))
    fmt match {
      case TIFF | DTED =>
        ingest
      case _         =>
  ...
  def ingest() {
    ...
    val args: Map[String, Option[String]] = Map(
      ...
      IngestRasterParams.FORMAT -> Some(Option(params.format).getOrElse(getFileExtension(new File(params.file)))),
